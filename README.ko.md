...
- **설치 대기열**: 검증된 부분 설치 파일 재개와 여러 Studio Pro 버전 대기열, 순서 변경·취소·재시도·재시작 복구 지원 ([설치 대기열 가이드](docs/install-queue.md))
...
네이티브 FreeRDP 프로세스가 선택 디렉터리를 읽을 수 있어야 합니다. sandbox/Flatpak FreeRDP wrapper를 사용한다면 패키징 시스템의 파일시스템 권한 설정에서 해당 디렉터리만 명시적으로 허용하고 다시 선택하세요. Mendimaru는 이를 해결하기 위해 홈 전체를 공유하지 않습니다. Windows 세션에 쓰기 가능한 drive를 제공하므로 현재 배포판에서 보안 수정된 FreeRDP 3을 사용하세요.
