> Historical archive. The active investigation is in [the current report](../winboat-studio-uia-spike.md). Checklists and unverified claims below describe their dated checkpoints.

# Studio Pro UI automation spike — Linux + WinBoat (#16)

## Active live investigation — 2026-09-15/16

The user confirmed #148 had reached PR verification and authorized continuing
#16. The previous guest hold below is historical. Native Windows stays deferred.
**No representative 20-trial campaign has begun; no feasibility decision or
merge is justified yet.** These are retained pilot and failure-boundary probes.

- Isolated X11 display and same-session helper established at **1280×800,
  96 DPI**, Studio **English (United States), Light**. Guest session 2,
  Windows build 26100, FreeRDP 3.31.1. Discover actual Docker port mappings;
  persisted configured ports differed from the running container.
- A disposable, local-only Blank Web App was exercised in **10.24.26.0**.
  Original product projects remain untouched. Studio 10 was saved and closed;
  a byte-identical MPR copy in a separate fixture directory is being opened in
  **11.12.4.0**. Its conversion and compatibility have not yet been verified.
  Saved source MPR SHA-256:
  `489c356920d21e577ba7c6b696a33191da2fc9945722aef23e5f90305c9cc89a`.
- Design preview exposes its page document, body text and mode buttons.
  Properties exposes an unnamed ValuePattern editor through the `Name` row
  label. Name change/save was independently verified in a copied MPR with
  mxcli. Reacquire after commit: old winapp slugs became stale.
- Raw .NET UIA and FlaUI 5.0.0 each captured **982 nodes without truncation**
  on the same later 10.x Design/Properties screen; matching editable name
  values were present. Earlier FlaUI and transport timeouts are retained.
  These differently instrumented single captures are not a speed comparison.
- The original depth 18 collector could miss WebView nodes. Depth 48 captured
  the full 10.x page tree, including nodes at depth 40. Node/time budgets remain
  explicit. The supervisor's stale-identity and five-second timeout paths were
  exercised; both rejected the request and reported the worker had exited.
- F4/F5 with WebView focus initially had no verified effect. Native WPF focus
  enabled synchronization and execution. The File menu is a native focus
  target. F4 can leave RemoteApp's marker window foreground; an input guard
  rejected that state, and native focus reacquisition recovered it.
- F4 removed/restored the unused Rating item. Removing a used Data grid 2
  package produced **10 consistency errors** with readable messages; restoring
  the hash-verified package returned **0 errors**. Error rows are virtualized;
  the initial tree exposed the visible subset, not all ten rows at once.
- F5 reached Run Project and then an actual runtime: guest HTTP 200, a
  `javaw.exe` child of the fixture's Studio PID owning port 8080, and the Console
  startup-complete message agreed. The semantic Stop button terminated it.
- LockWorkStation succeeded. The input desktop became unavailable; .NET
  capture rejected it and winapp input returned `no_interactive_desktop`.
  Reconnection preserved Studio PID/start ticks and session 2.
- At **125%**, the monitor and a new helper reported **120 DPI**, while the
  already-running Studio still reported **96 DPI**. Per-monitor-aware widget
  selection, name change/readback/restoration and both modes passed. Collectors
  now record monitor scale/system DPI as well as target-window DPI. The RDP
  connection was returned to 100% before opening Studio 11.
- Windows PrintWindow/WGC contained the WebView while an owned notification
  obscured the Linux screen. WGC's multi-window output is a side-by-side
  composite, not a picture of screen occlusion. Linux on the isolated **xfwm4
  compositor-off** display showed the notification's transparent area as black;
  compositor-on behavior is not yet tested. Minimize/restore left Linux output
  blank while Windows captures still rendered; reconnect recovered it. Do not
  generalize that one pilot into an availability guarantee or a diagnosed cause.
- Private requests, trees and transport diagnostics remain in the dedicated
  `issue-16/live-20260915` state and disposable shared lab. Pilot scripts are
  also copied into its private `pilot-scripts` directory. The experimental
  bridge had serialization, timeout-race and sequence-replay bugs; incomplete
  attempts and their diagnostics are retained. It is not a product provider.
- Other verification VMs/builds caused observed host memory pressure. Pilot
  latency is not a candidate performance comparison. Reviewed, non-private
  [pilot screenshot examples](artifacts.json) are retained
  separately from the still-unstarted campaigns.

Continue with the 11.x fixture verification, remaining transport/candidate
checks, a frozen representative flow and **20 actual trials per exact version**.
Restore the original **Korean / Auto** Studio preferences at the end. Complete
live acceptance gates before replacing this checkpoint with the final Linux-only
report, PR and verified merge. Keep native Windows explicitly deferred.

## Historical checkpoint: 2026-09-15, tools prepared; #148 uses the guest first
